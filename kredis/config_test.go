...
func TestConfig_SetLogger(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		testbigdata[randSeq(5)] = "This is a kredis client GUI tool written based on Java SWT and Jedis. It's my objective to build the most convenient kredis client GUI tool in the world. In the first place, it will facilitate in editing kredis data, such as: add, update, delete, search, cut, copy, paste etc." // randSeq(355)
	}
